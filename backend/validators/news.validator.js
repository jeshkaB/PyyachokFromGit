...
const newsTitleValidator = Joi.string().min(3).max(120).trim().error(new LocalError('title is not valid', BAD_REQUEST));
const newsContentValidator = Joi.string().min(240).max(2000).error(new LocalError('content is not valid', BAD_REQUEST));
