...
const restaurantNameValidator = Joi.string().max(20)
  .trim()
  .error(new LocalError('name is not valid', BAD_REQUEST));
const placeValidator = Joi.string().trim()
  .error(new LocalError('place is not valid', BAD_REQUEST));
...
const restaurantEmailValidator = Joi.string().email()
  .error(new LocalError('email is not valid', BAD_REQUEST));
const moderatedValidator = Joi.boolean().error(new LocalError('moderated is not valid', BAD_REQUEST));
const moderationMessageValidator = Joi.string().min(3)
  .max(50)
  .error(new LocalError('moderation message is not valid', BAD_REQUEST));
const coordinatesValidator = Joi.array().ordered(
  Joi.number().min(coordinatesLimit.LONGITUDE_MIN)
    .max(coordinatesLimit.LONGITUDE_MAX),
  Joi.number().min(coordinatesLimit.LATITUDE_MIN)
    .max(coordinatesLimit.LATITUDE_MAX))
