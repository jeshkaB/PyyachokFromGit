...
module.exports = {
  ...
  checkPasswordPairIsValid: (req, res, next) => {
    try {
  ...
  checkPasswordIsValid: (req, res, next) => {
    try {
  ...
  checkSuperAdminIsNotExist: async (req, res, next) => {
    try {
      const users = await userService.getUsers();
      const superAdmin = users.find(user=>user.role.includes(roles.SUPER_ADMIN));
      if (superAdmin) {
        return next(new LocalError('Superadmin is already exist', statusCodes.CONFLICT));
      }
      next();
    } catch (e) {
      next(e);
